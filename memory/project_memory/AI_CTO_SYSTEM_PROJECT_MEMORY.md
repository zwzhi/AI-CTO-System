...
- 可移植性、Skill 包、安装器、文档引用和 `212 / 212` 回归验证已通过；当前提交将标记为本地 `v2.0.0-codex-native`。未授权前不推送远程、不修改 `main`。Execution Authorization 仍为 `NONE`。
...
> 2026-08-06：用户选择方案 A 并批准 `SYS-L5-SKILL-GATEWAY-001` Global AI CTO Skill Gateway 设计：权威 Skill 源保留在 `D:\AI Project\AI-CTO-System\skills\ai-cto-system`，用户级 `~.codex\skills\ai-cto-system` 通过 NTFS Junction 全局发现。入口采用轻量触发与按需加载；`AI_CTO_MODE: OFF`、“不要使用 AI CTO System”、“普通模式处理”和“本次禁用 AI CTO Skill”优先于自动触发。归类为 Layer 5 现有 Intent Gateway / Runtime 入口扩展，不新增 Module、Phase、Plugin 或 Runtime Capability。当前只完成设计，未安装，且本地仓库没有 Git remote、未上传 GitHub。
...
- Global AI CTO Skill Gateway 必须在加载治理权威前先执行最小路由预检：`L0` 普通处理，`L1` 使用 Instant / LIGHT / R1 / TARGETED，`L2` 使用 Engineering / STANDARD / R2，普通 `L3` 使用 Design + Engineering / STANDARD / R3 / CHANGE_IMPACT_AND_TARGETED，`L4` 使用 CTO / STRICT / R4 / FULL_GATE；风险、不可逆性、当前 Evidence 要求和适用 Gate 可独立将档位升级为 STRICT / FULL_GATE。触发 Skill 本身不能提高任务等级；Context 表是允许范围，不是预加载清单；Reasoning 与模型条目仍是建议，不代表宿主模型切换或工具授权。
...
- 2026-08-06：Global AI CTO Skill Gateway 已合并到 `main` 并在本机安装。用户级 `~.codex\skills\ai-cto-system` 是指向仓库 `skills\ai-cto-system` 的 NTFS Junction，重复安装验证为幂等，canonical source 和 metadata 可读。当前状态严格保持 `INSTALLED_AWAITING_FRESH_SESSION_PILOT`；当前对话的 Skill catalog 不能充当启动发现 Evidence，需在新 Codex 对话验证四类 Pilot 后再决定是否标记 `GLOBAL_SKILL_GATEWAY_VALIDATED`。
...
- Fresh Session host behavior is `NOT_CAPTURED`: the implementation branch is merged into canonical `main` and the user-level Junction targets that Skill source; the five-case pilot must be run in a newly opened Codex conversation.
- Next Action: run and record the Fresh Session pilot without inferring host selection from static files.
