...
本模板建立 Requirement ID → Design ID / 设计文档 → Development Task ID → Commit SHA → Test Case ID 的五层身份链，并把实际 Test Evidence 关联到对应 Test Case，是 [文档关联规则](../protocol/DOCUMENT_RELATIONSHIP.md) 定义的权威追踪索引。源文档仍是具体内容的单一事实来源，矩阵只保存稳定 ID、版本化引用、状态和影响关系。

完成后的矩阵同时用于 [Design Approval Gate](./DESIGN_APPROVAL_GATE.md)、[Development Approval Gate](../development/DEVELOPMENT_APPROVAL_GATE.md) 与 [Testing Release Gate](../release/TESTING_RELEASE_GATE.md)。它必须支持从需求到测试证据的正向追踪，以及从证据、测试、Commit、任务或设计回到需求的反向追踪。

## 2. 使用规则

1. 已批准需求使用稳定且唯一的 REQ-XXXX；设计使用 DES-XXXX；开发任务使用 TASK-XXXX；Git 提交使用不可变 Commit SHA；测试案例使用 TC-XXXX；测试执行证据使用 TE-XXXX。ID 废弃后不得复用。
...
6. 每个 Task ID 必须同时引用其实现的 Requirement ID 和 Design ID；每个 Commit 必须引用 Task ID；每个 Test Case ID 必须引用其验证的 Requirement ID、Design ID、Task ID 和相关 Commit；每个 Test Evidence ID 必须引用 Test Case、精确候选基线与原始结果。
...
- 申请 TESTING → RELEASE 时，每项发布必需 Test Case 必须关联当前候选基线的实际 TE-XXXX，结果达到预设通过条件；`NOT_RUN`、FAIL、BLOCKED、INVALIDATED 或失效 Evidence 均不能作为发布证据。
...
| 对应 Development Gate 记录 | {DEVELOPMENT_GATE_RECORD_REFERENCE} |
| 对应 Testing Release Gate 记录 | {TESTING_RELEASE_GATE_RECORD_REFERENCE_OR_NOT_CREATED} |
...
| Test Strategy | {TEST_STRATEGY_LINK} | {TEST_STRATEGY_VERSION} | {STATUS} | {APPROVER} | {YYYY-MM-DD} | Applicable |
| Bug Register | {BUG_REGISTER_LINK} | {BUG_REGISTER_VERSION} | {STATUS} | {APPROVER} | {YYYY-MM-DD} | Applicable |
| AI Evaluation | {AI_EVALUATION_LINK_OR_NA_RECORD_LINK} | {VERSION_OR_NA_RECORD_VERSION} | {STATUS} | {APPROVER} | {YYYY-MM-DD} | {APPLICABLE_OR_AUDITED_NA} |
| Security Review | {SECURITY_REVIEW_LINK} | {SECURITY_REVIEW_VERSION} | {STATUS} | {APPROVER} | {YYYY-MM-DD} | Applicable |
...
只有当前批准范围内的全部链按当前阶段规则均为 ALIGNED，才可通过相应门禁。Design Approval Gate 允许 Commit 为 `NOT_CREATED`、测试为计划状态；Development Approval Gate 还要求实际 Commit、Review 与开发级测试证据完整；Testing Release Gate 要求全部发布必需测试具有当前基线的有效 Evidence。IMPACT_REVIEW_REQUIRED、BROKEN 或未经批准的 RETIRED 在所有门禁中均为阻断项。

| Chain ID | Requirement ID | Requirement 文档链接 @ 版本 / 章节 | Design ID | Design 文档链接 @ 版本 / 章节 | Development Task ID | Development Plan 链接 @ 版本 / 章节 | Commit SHA / 状态 | Commit 证据链接 | Test Case ID | Test Plan 链接 @ 版本 / 章节 | Test Evidence ID / 链接 | Test Result | Chain Status |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| CHAIN-0001 | REQ-0001 | {REQUIREMENT_REFERENCE} | DES-0001 | {DESIGN_REFERENCE} | TASK-0001 | {TASK_REFERENCE} | {COMMIT_SHA_OR_NOT_CREATED} | {COMMIT_REFERENCE_OR_PENDING} | TC-0001 | {TEST_REFERENCE} | {TEST_EVIDENCE_REFERENCE_OR_NOT_CREATED} | {TEST_RESULT} | {CHAIN_STATUS} |

## 6. 链路状态与验证记录

源状态必须使用源文档定义的词汇；Test Result 至少区分 NOT_RUN、PASS、FAIL、BLOCKED、INVALIDATED。Design Approval Gate 前，计划在 DEVELOPMENT 或 Phase 6 执行的测试可以为 NOT_RUN，但 Test Case 必须完整、可执行并有明确通过条件。Development Approval Gate 前，开发级必跑测试必须 PASS；Phase 6 的完整系统、非功能、正式评测与用户验收用例可以保持 NOT_RUN，但其范围、环境、数据、责任人和判定标准必须就绪。Testing Release Gate 前，全部发布必需用例必须拥有当前精确基线的有效 Evidence 并达到预设通过条件。

| Chain ID | Requirement 状态 | Design 状态 | Task 状态 | Commit / Review 状态 | Test Case 状态 | Test Result | Test Evidence ID | 验证环境 / 证据链接 | 责任人 | 最后核验日期 |
|---|---|---|---|---|---|---|---|---|---|---|
| CHAIN-0001 | {REQUIREMENT_STATUS} | {DESIGN_STATUS} | {TASK_STATUS} | {COMMIT_AND_REVIEW_STATUS} | {TEST_CASE_STATUS} | {TEST_RESULT} | {TEST_EVIDENCE_ID_OR_NOT_CREATED} | {EVIDENCE_REFERENCE} | {OWNER} | {YYYY-MM-DD} |

### 6.1 Test Evidence 登记

Evidence 是一次测试执行的不可覆盖记录，不是 Test Case 的副本。重新执行必须创建新 TE-XXXX，并把旧记录标记为历史或 INVALIDATED；不得覆盖失败结果。

| Test Evidence ID | Test Case ID | Requirement ID | Design ID | Task ID | Commit / Release Candidate | 环境与配置 | 测试数据版本 | 执行时间 / 执行人 | 预期结果 | 实际结果 | Result | 原始证据链接 | 有效性 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| TE-0001 | TC-0001 | REQ-0001 | DES-0001 | TASK-0001 | {COMMIT_OR_CANDIDATE} | {ENVIRONMENT_AND_CONFIG} | {TEST_DATA_VERSION} | {TIMESTAMP_AND_EXECUTOR} | {EXPECTED_RESULT} | {ACTUAL_RESULT} | {PASS_FAIL_BLOCKED} | {RAW_EVIDENCE_REFERENCE} | {CURRENT_OR_INVALIDATED} |

Test Evidence 必须足以让独立评审人确认测试对象、输入、步骤、预期、实际和判定；截图、口头结论或只写“测试通过”不能单独作为 Evidence。Requirement → Test Case → Evidence 的每条发布必需投影都必须完整。

## 7. 正向追踪检查

正向检查从每个已批准 Requirement ID 出发，确认其所有设计、任务和测试分支均在主矩阵中展开。

| Requirement ID | 源文档版本 | 预期 Design ID | 对应 Chain ID | 全部 Task 已覆盖 | 全部 Commit 已覆盖 | 全部 Test Case 已覆盖 | 全部必需 Evidence 已覆盖 | 结果 | 证据 / 说明 |
|---|---|---|---|---|---|---|---|---|---|
| REQ-0001 | {REQUIREMENT_VERSION} | DES-0001 | CHAIN-0001 | {YES_OR_NO} | {YES_NO_OR_NOT_CREATED} | {YES_OR_NO} | {YES_NO_OR_NOT_RUN} | {PASS_OR_FAIL} | {EVIDENCE_REFERENCE} |
...
- 申请 TESTING → RELEASE 时，每个发布必需 Test Case ID 至少有一个绑定当前候选基线且有效的 Test Evidence ID。
- 一项需求存在多个设计、任务或测试时，所有分支都已列出，不能只验证其中一条代表链。

## 8. 反向追踪检查

反向检查从每个 Test Evidence ID 出发回溯到 Test Case、Commit、Task、Design 和 Requirement，随后分别从所有 Test Case、Commit、Task ID 与 Design ID 再做一次上游检查，防止无需求依据的实现、测试或结果进入范围。

| 起点类型 | 起点 ID | 对应 Chain ID | 回溯 Test Case ID | 回溯 Commit SHA | 回溯 Task ID | 回溯 Design ID | 回溯 Requirement ID | 引用版本有效 | 结果 | 证据 / 说明 |
|---|---|---|---|---|---|---|---|---|---|---|
| Test Evidence | TE-0001 | CHAIN-0001 | TC-0001 | {COMMIT_SHA} | TASK-0001 | DES-0001 | REQ-0001 | {YES_OR_NO} | {PASS_OR_FAIL} | {EVIDENCE_REFERENCE} |
| Test Case | TC-0001 | CHAIN-0001 | TC-0001 | {COMMIT_SHA} | TASK-0001 | DES-0001 | REQ-0001 | {YES_OR_NO} | {PASS_OR_FAIL} | {EVIDENCE_REFERENCE} |
| Commit | {COMMIT_SHA} | CHAIN-0001 | TC-0001 | {COMMIT_SHA} | TASK-0001 | DES-0001 | REQ-0001 | {YES_OR_NO} | {PASS_OR_FAIL} | {EVIDENCE_REFERENCE} |
| Development Task | TASK-0001 | CHAIN-0001 | TC-0001 | {COMMIT_SHA_OR_NOT_CREATED} | TASK-0001 | DES-0001 | REQ-0001 | {YES_OR_NO} | {PASS_OR_FAIL} | {EVIDENCE_REFERENCE} |
| Design | DES-0001 | CHAIN-0001 | TC-0001 | {COMMIT_SHA_OR_NOT_CREATED} | TASK-0001 | DES-0001 | REQ-0001 | {YES_OR_NO} | {PASS_OR_FAIL} | {EVIDENCE_REFERENCE} |

反向检查通过条件：

- 每个 Test Case ID 在进入 TESTING 前都能回溯到至少一个 Commit SHA、Task ID、Design ID 和 Requirement ID。
- 每个 Test Evidence ID 都能回溯到当前 Test Case、Commit 或 Release Candidate、Task、Design 和 Requirement，且执行环境与数据版本可复核。
...
| ORPHAN-EVIDENCE | Test Evidence 无 Test Case、候选基线或 Requirement / Design / Task 上游 | 从全部 TE-XXXX 反向检查 Test Case 与五层链 | {INTEGER} | {PASS_OR_FAIL} | {EVIDENCE_REFERENCE} | {RESOLUTION_REFERENCE} |
| STALE-EVIDENCE | Evidence 对应旧 Commit、配置、环境、模型、Prompt、工具或数据集 | 对比 Test Evidence 与当前候选基线及变更影响记录 | {INTEGER} | {PASS_OR_FAIL} | {EVIDENCE_REFERENCE} | {RESOLUTION_REFERENCE} |
...
| Test Evidence | {INTEGER} | {INTEGER} | {INTEGER} | Not Applicable | {INTEGER} | {PASS_OR_FAIL} |

完整性检查只有在以下条件全部满足时才为 PASS：

- 当前阶段适用的五层活动项总数均已核对，主矩阵覆盖数量与范围一致。
- Requirement 的下游、Design 与 Task 的双向、Test Case 的上游完整率均为 100%。
- 申请 TESTING → RELEASE 时，发布必需 Test Case 的有效 Evidence 覆盖率为 100%，Requirement → Test Case → Evidence 投影无缺口。
...
| Requirement → Test Case → Evidence 完整 | {PASS_OR_FAIL} | {REVIEWER} | {YYYY-MM-DD} | {EVIDENCE_REFERENCE} |
