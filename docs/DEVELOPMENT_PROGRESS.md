...
| Advisory Router 实现完成 | 2026-08-06 | 用户授权后按 TDD 实现 `EvidenceFreshnessService`、`ExecutionProfilePolicy` 与 `AdvisoryExecutionRouter`。9 项目标测试和全量 135 项回归通过，范围扫描无文件/Git/网络、模型/工具、Workflow 或 Audit Repository 依赖。Development Gate 为 `APPROVED_FOR_TESTING`；不代表真实模型选择、自动切换、工具调用或执行授权。 |
