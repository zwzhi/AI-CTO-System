...
Capability Governance 的文档治理在 Phase 8.2 完成；Engineering Capability Strategy 与 Roadmap 已定义 Provider 无关的类别、合同、优先级、依赖、人类控制和 `Adopt / Improve / Merge / Deprecate / Remove` 策略动作。真实 Capability Record、自动选择、插件接入和工具调用仍未实现。`Completed` 不表示已安装或激活任何外部能力。
