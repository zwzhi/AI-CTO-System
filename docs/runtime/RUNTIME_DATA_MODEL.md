...
`CREATED`、`PLANNING`、`WAITING_APPROVAL`、`EXECUTING`、`VALIDATING`、`COMPLETED`、`FAILED`、`ROLLING_BACK`、`PAUSED`、`CANCELLED`。

MVP 正常路径使用 `CREATED → PLANNING → EXECUTING → VALIDATING → COMPLETED`。`FAILED` 与 `CANCELLED` 为受控终态；`ROLLING_BACK` 为受控回滚过程状态，完成后进入 `CANCELLED` 或经授权恢复。`PAUSED` 只能经授权恢复。MVP 不实现真实回滚动作，只记录需要回滚的状态与 Evidence。
