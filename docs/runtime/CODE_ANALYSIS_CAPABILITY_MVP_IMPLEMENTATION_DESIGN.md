...
`L3` requires every context to contain a non-empty `versionRef`; otherwise the highest possible result is `L2`. The adapter rejects a port result with missing fields, unapproved references, altered evidence, or confidence above this bound.
...
Failure is returned as `BLOCKED` for preflight rejection and `FAILURE` for invocation or result-validation failure. No successful result is returned for a failed validation. Evidence summaries and result text must not echo full authorised source content.
