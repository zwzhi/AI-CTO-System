...
| Layer 5 | Capability Governance 的准入、Registry、评估、选择、生命周期和外部接入规范 | Intent Gateway、Agent Runtime、Tool Calling、Codex Integration、Automation、真实 Capability 接入与自动选择均为 `Planned` |
...
| 当前工作 | Phase 8.5 Intent Gateway Governance 文档治理 | 建立建议性 Intent Classification Result 合同；不新增 Runtime 或自动化 |
